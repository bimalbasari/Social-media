...
const EventPage = ({ data }) => {
  ...
  return (
    <div className="bg-white  md:w-2/4 m-auto  h-full  border-double border-4 border-slate-700 mt-4 rounded-lg overflow-hidden">
      ...
      <div className="p-4">
        <p className="text-md leading-4 md:text-md md:leading-5 text-blue-600">
          {data.content}
        </p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between px-12 border-b-2 m-1">
          <div
            className="font-medium text-sm text-cyan-800  mx-2 cursor-pointer"
            onClick={showLike}
          >
            {likes}
          </div>
        ...
        <div className="h-12 flex items-center justify-between px-12 ">
          ...
          <div className="flex items-center cursor-pointer" onClick={onLike}>
            ...
            <span className="font-medium text-sm text-cyan-800  mx-2">
              Like
            </span>
          </div>
          ...
          <div className="flex items-center focus:outline-none disabled:opacity-25 cursor-pointer">
            ...
            <span className="font-medium text-sm text-cyan-800 disable mx-2 ">
              Repost
            </span>
          </div>
        </div>
        {commentBox && (
          <Comment user={user} data={data.comments} id={data._id} />
        )}
      </div>
    </div>
  );
